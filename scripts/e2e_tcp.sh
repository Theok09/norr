#!/bin/bash
# Two Norr nodes over the TLS-protected TCP carrier.
#
...
cleanup() {
  for ns in tcp-a tcp-b; do
    ip netns pids "$ns" 2>/dev/null | xargs -r kill 2>/dev/null
  done
  sleep 0.2
  for ns in tcp-a tcp-b; do ip netns del "$ns" 2>/dev/null; done
}
trap cleanup EXIT
cleanup

ip netns add tcp-a || exit 1
ip netns add tcp-b || exit 1
ip link add ca netns tcp-a type veth peer name cb netns tcp-b
ip netns exec tcp-a ip addr add 10.26.0.1/24 dev ca
ip netns exec tcp-a ip link set ca up; ip netns exec tcp-a ip link set lo up
ip netns exec tcp-b ip addr add 10.26.0.2/24 dev cb
ip netns exec tcp-b ip link set cb up; ip netns exec tcp-b ip link set lo up

ip netns exec tcp-b ping -c1 -W2 10.26.0.1 >/dev/null 2>&1 \
  && ok "carrier reachable" || { bad "carrier unreachable"; exit 1; }

mkdir -p /tmp/tcpe2e && cd /tmp/tcpe2e || exit 1
...
ip netns exec tcp-a "$NORR" run /tmp/tcpe2e/a.toml >a.log 2>&1 &
ip netns exec tcp-b "$NORR" run /tmp/tcpe2e/b.toml >b.log 2>&1 &
sleep 3

ip netns exec tcp-a ip link show tcpA >/dev/null 2>&1 \
  && ok "listener side created its TUN device" || bad "listener has no TUN"
ip netns exec tcp-b ip link show tcpB >/dev/null 2>&1 \
  && ok "dialing side created its TUN device" || bad "dialer has no TUN"

# The TCP connection itself must be established before anything else can work.
if ip netns exec tcp-a ss -tn 2>/dev/null | grep -q 51840; then
  ok "TCP connection established"
else
  bad "no TCP connection"
fi

ip netns exec tcp-a ip addr add 10.34.0.1/32 dev tcpA 2>/dev/null
ip netns exec tcp-a ip link set tcpA up
ip netns exec tcp-a ip route add 10.34.0.2/32 dev tcpA 2>/dev/null
ip netns exec tcp-b ip addr add 10.34.0.2/32 dev tcpB 2>/dev/null
ip netns exec tcp-b ip link set tcpB up
ip netns exec tcp-b ip route add 10.34.0.1/32 dev tcpB 2>/dev/null
sleep 4

if ip netns exec tcp-b ping -c3 -W3 -I 10.34.0.2 10.34.0.1 >ping.log 2>&1; then
